...
graph, calibrated by a repo benchmark script against a real corpus and cut on the
conservative side of the measured optimum (split rather than merge); the greedy
per-observation assignment stays as the *incremental* path, cheap to be wrong on precisely
because the rebuild exists. ADR-0017.
