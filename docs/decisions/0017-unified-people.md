...
The faces pipeline is video-only end to end:

| Site | Video-only behaviour |
|---|---|
| `adapters/db/global-catalog.ts:1086` `listFaceIndexCandidates` | walks `folders`/`files`; a photo fingerprint can never become a candidate |
...
| `adapters/db/global-catalog.ts:1287` `faceStatus` | `staleVersionFiles` counts `face_index_state` rows, a video-only table; `filesIndexed` mixes both media into one number that reads as "videos" |
| `adapters/db/global-catalog.ts:2132` collection person filter | `EXISTS (SELECT 1 FROM face_observations o WHERE o.fingerprint = f.fingerprint …)` — the join is against `files`, so it is structurally video-only |
...
| `adapters/db/global-catalog.ts:804` `listLibraryFacets` | counts people over *all* observations including photos — so the Osoby facet promises a count the person filter then refuses to deliver |
...
The result the owner sees: Osoby offers person chips whose counts include
photos, clicking one silently drops every photo from Kolekcja, and no photo
ever shows who is in it.

### The 24,322 imported photo observations

`core/server/usecases/photo-import-libra.ts:290` wrote PHOTO LIBRA's face
records straight into the shared pool with `media: 'photo'`, `personId: null`,
`cropPath: null`, `frameTsS: 0`, and marked each source photo complete via
`completePhotoFaceIndex(fingerprint, FACE_ENGINE_VERSION)`.

The received brief for this ADR said those embeddings come from a *different
model*. **They do not** — this was checked, not assumed. PHOTO LIBRA's
`faces-index.ts` pins the same two artifacts by the same sha256 that
`core/domain/models.ts:48-61` pins: YuNet `2023mar`
(`8f2383e4…52fa4`) and SFace `2021dec` (`0ba9fbfa…c34e79`), 128 floats,
L2-normalized. The correct statement of the problem is narrower and still
decisive:

- **No crops.** `crop_path` is NULL for all 24,322 rows. [ADR-0014](0014-per-observation-face-crops.md)
  makes the per-observation crop the thing a person card is built from, and
  the only repair path (`faces exemplars`) re-detects through video
  timestamps, so it cannot repair a photo row. A cluster made only of imported
  photo observations is a person with no face to show.
- **Foreign geometry.** The bboxes are in PHOTO LIBRA's own proxy space, and
  neither those proxies nor their dimensions are recorded on the row. They
  cannot be re-cut from `photo-artifacts/proxies/<fingerprint>.jpg`, whose
  generation pipeline is a different one.
- **Foreign alignment.** Same checkpoint is not the same vector: PHOTO LIBRA
  aligns with its own `warpAlignedFaceRgb` before the embedder, gated on its
  own quality thresholds. Nothing in the repo proves the two alignment paths
  agree, and an identity pool is only as good as the assumption that every
  vector in it was produced the same way.
- **Foreign obs numbering.** `<md5>:face:<n>` was translated to
  `<fingerprint>:face:1:<n>` with libra's detection order. A native pass
  renumbers by its own detection order, so the two schemes collide rather than
  merge.
...
### What the calibration corpus really is

The owner's decision names PHOTO LIBRA's `face-assignments.json` +
`people.json` as the calibration ground truth. Measured, in
`~/repositories/claude-tmp/photo-libra-sort/`:

- `faces.ndjson` — 42,395 lines, one per photo libra ran detection over;
  25,327 of them carry a complete observation (obsId + bbox + 128-float
  embedding), the rest are `{"md5": …, "obsId": null}` (no face found).
- `face-assignments.json` — 25,327 entries: 23,203 assigned to a person,
  1,725 `"review"`, 399 `null`.
- `people.json` — 2,060 clusters, largest 1,165 observations, **no
  singletons** (libra ran `newClusterMinObservations: 3`), exactly one named:
  `person-0001` = "Mateusz Choma".
- `faces-ui-log.ndjson` — **8 lines total**: five `name` actions (three of
  them `__faces_ui_verification__` noise) and three `merge` actions, all into
  `person-0001`.

So the corpus is **not** 42,395 human-assigned observations. It is the output
of libra's own greedy clusterer at `autoAssignSimilarity: 0.45` /
`newClusterSimilarity: 0.5` / `newClusterMinObservations: 3`, touched by
exactly four human decisions. Calibrating a new threshold to maximise
agreement with it would reproduce a 0.45 greedy assigner, which is the very
algorithm this ADR replaces. The owner's decision to use it stands — it is by
far the largest labelled-ish partition available — but this ADR binds the
benchmark to treat it as a **reference partition**, never as truth, and adds a
small owner-labelled pair sample as the deciding evidence (D4 below).
...
(`core/server/ports.ts:934`), writes observations with `media: 'photo'`, frame
...
**D2 — The imported PHOTO LIBRA observations are dropped, not reconciled.**
A one-shot migration deletes every `face_observations` row with
`media = 'photo'` (at migration time these are, by construction, exactly the
libra rows — no native photo row can exist yet) and clears the matching
...
- **Sparse by construction.** The candidate edge set is every pair with cosine
  ...
  look worse than it is, which is exactly the split-rather-than-merge bias the
  owner asked for.
...
- *fixture mode* — small synthetic embedding fixtures committed under
  ...
  gated by `check`;
- *real-data mode* — points at the PHOTO LIBRA corpus and the app's own
  native observations, matching libra observations to native detections by
  photo (manifest `md5` → `ph_` fingerprint) and **bbox IoU ≥ 0.5** (reusing
  `EXEMPLAR_BBOX_MIN_IOU`, `core/domain/faces.ts:37`); never part of `check`.

Because the reference partition is machine-made (see Context), real-data mode
also emits a stratified pair sample (equal counts per 0.02 similarity band
across 0.30–0.70) for the owner to label same/different. The chosen threshold
must (a) sit at or above the pairwise-F1 optimum against libra's partition and
(b) produce **zero** owner-labelled different-person pairs inside a merged
cluster. If (a) and (b) disagree, (b) wins.
...
dropped for this rebuild: with one name in the whole corpus it buys nothing
and it hides membership changes behind a familiar label. Every person after
the rebuild is unnamed; the owner renames from the person cards. `namesDropped`
still reports what was lost.
...
**D8 — Counters tell the truth per medium.** `facesStatusOutputSchema` gains
additive per-medium counts (`videosIndexed`, `photosIndexed`,
`stalePhotoFiles`) alongside the existing fields, which keep their current
meaning; `filesIndexed` stops being a number that reads as videos and is
documented as the union.

## Alternatives rejected

- **Backfill crops for the imported observations instead of re-detecting.**
  Would need libra's proxies (not in the app's artifact tree), libra's bbox
  space and a trusted alignment equivalence — three unverifiable assumptions
  to save one detection pass over photos whose proxies are already on disk.
...
- **Keep the greedy assigner and only re-tune its thresholds.** The greedy
  ...
  order-independent, and the benchmark it is calibrated against would then be
  measuring its own family.
- **HDBSCAN / DBSCAN.** Density-based clustering leaves a noise class the
  product has no surface for, and its parameters are less legible to the owner
  than a single cosine cut.
- **Complete linkage.** Splits harder than the owner asked for and is
  dominated by single high-quality outliers within a person; average linkage
  with the non-edge-as-zero rule already carries the conservative bias.
...
2. The owner runs the photo faces pass over the photo root. Every photo is a
   candidate again; observations are written with native geometry and crops.
   People are assigned incrementally by the existing greedy path, which is
   expected to be wrong at the seams — that is what step 3 is for.
3. `faces recluster --dry-run` from Osoby; the owner reads the report.
4. `faces recluster` for real. All person ids are re-minted; all names are
   gone (D6).
5. The owner renames the people that matter.
...
- **The V16/v6 migration does not revert**, which is why it dumps to
  ...
  the rows are exactly the ones the owner decided are worthless.
...
- Person ids change once more, and this time every name is dropped by design
  (exactly one name exists to lose). The owner re-names from person cards.
- Recluster stops being O(N × clusters) and becomes O(edges) over a sparse
  neighbour graph, with a chunked block pass to find those edges. At the
  owner's scale (~50k observations, 128 floats) the edge pass is the cost
  driver; the F2 benchmark must report wall-clock time, and if a full rebuild
  exceeds ten minutes the sanctioned fallback is a random-projection banding
  prefilter before the block pass — never a lower `reviewBandMin`.
