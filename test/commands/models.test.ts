...
describe('models command', () => {
  ...
  describe('models download', () => {
    it('should handle already downloaded model', async () => {
      // This test will likely show the model as already downloaded if it exists
      // or start a download if it doesn't
      const result = await runCli(['models', 'download', 'tiny', '--json'], {
        cwd: testDir,
        timeout: 60000, // longer timeout for potential download
      });

      // Should succeed (either downloaded or already exists)
      expect(result.exitCode).toBe(0);
