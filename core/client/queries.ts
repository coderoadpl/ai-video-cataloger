 ...
 * types (never a React binding, which `core/client` may not import). They bind
 * the `queryFn` result type to the key so `useQuery`/`useMutation` infer
 * `data`/`variables` at the call site without explicit generics.
 *
...
/** Query keys are the public API of each resource: general → specific. */
