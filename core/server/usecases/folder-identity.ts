...
export const resolveFolderIdentity = async (
  ...
  const existing = await readFolderMarker(fs, folder);
  if (!existing.ok) {
    if (isReadOnlyWriteError(existing.error)) return ok({ folderId: pathFolderId(folder), persistent: false });
    return existing;
  }
  ...
  const ensured = await fs.ensureDirectory(fs.dirname(markerPath));
  if (!ensured.ok) {
    if (isReadOnlyWriteError(ensured.error)) return ok({ folderId: pathFolderId(folder), persistent: false });
    return ensured;
  }
  const written = await fs.writeTextFile(markerPath, JSON.stringify(marker, null, 2));
  if (!written.ok) {
    if (isReadOnlyWriteError(written.error)) return ok({ folderId: pathFolderId(folder), persistent: false });
    return written;
  }
  return ok({ folderId: marker.folderId, persistent: true });
};

const pathFolderId = (folder: string): string => {
