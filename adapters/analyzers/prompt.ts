...
export const filenameInstruction =
  'a lowercase kebab-case filename (no dates, no extension) built so a future search lands on this exact clip: most distinctive verifiable detail first - proper names, text you read, place, subject, action. Three words when that is honestly all the clip offers, up to eight when the content earns them. Never use filler words such as video, clip, footage, recording, scene or movie, and never settle for a generic label like museum-exhibit or boat-display when anything more specific is visible.';
