...
These are the sites this plan changes. Line numbers are as of `89a2cfa`.

| # | Site | Today | Phase |
|---|---|---|---|
| 1 | `adapters/db/global-catalog.ts:1086` `listFaceIndexCandidates` | walks `folders`/`files`; photos can never be candidates | F1 (left alone — the photo leg uses `listPhotoFaceIndexCandidates` instead) |
...
| 3 | `adapters/db/photos-store.ts:398` | skips any photo whose `photo_face_index_state.engine_version >= FACE_ENGINE_VERSION` — i.e. all 24,322 libra-marked photos | F1 (unchanged code; the migration clears the rows) |
...
| 8 | `adapters/db/global-catalog.ts:1287` `faceStatus` | `staleVersionFiles` reads the video-only `face_index_state`; `filesIndexed` silently unions both media | F1 |
| 9 | `adapters/db/global-catalog.ts:2132` collection person filter | `EXISTS (… face_observations o WHERE o.fingerprint = f.fingerprint …)` joined against `files` | F3 |
...
| 11 | `adapters/db/global-catalog.ts:804` `listLibraryFacets` | counts people over all observations incl. photos — a promise #9/#10 refuse to keep | F3 (becomes honest once #9/#10 land; add the pinning test) |
...
| 16 | `core/contract/routes.ts:2000` `facesStatusOutputSchema` | no per-medium counters | F1 (additive fields only) |
...
geometry and crops; the 24,322 imported PHOTO LIBRA observations are gone.
...
writing one crop per observation like the video pass; the face observations
imported from PHOTO LIBRA are dropped on first open (backed up under
`~/.ai-video-cataloger/backups/`) because they carry no crops and foreign
geometry" + "`GET /api/faces/status` reports `videosIndexed`, `photosIndexed`
and `stalePhotoFiles` alongside the existing counters".
...
- `scripts/faces-benchmark.ts` (TypeScript, run with `node --experimental-strip-types`
  like the repo's other scripts): sweeps the cut threshold and reports
  **purity, completeness, pairwise F1, cluster-count and wall-clock** per
  threshold.
  ...
  - *real-data mode* (`--corpus <dir> --catalog <path>`): reads PHOTO LIBRA's
    `faces.ndjson` / `face-assignments.json` / `people.json` + the app's own
    native observations, matches libra observation → native observation by
    photo (`manifest.md5` → `ph_` fingerprint) **and** bbox IoU ≥
    `EXEMPLAR_BBOX_MIN_IOU`, and reports unmatched counts honestly. Never in
    `check`, never in `smoke`.
  - also emits a stratified same/different pair sample (equal counts per 0.02
    band across 0.30–0.70) for owner labelling, and scores a candidate
    threshold against the returned labels.
- The chosen threshold is written into `FACE_CLUSTERING` with the measured
  numbers quoted in the ADR consequences; it sits **at or above** the
  pairwise-F1 optimum against libra's partition, and it must admit **zero**
  owner-labelled different-person pairs into one cluster. If the two criteria
  disagree, the owner-labelled criterion wins.
...
5. Scale guard: a synthetic pool at the owner's order of magnitude clusters
...
PR; wall-clock for a full rebuild at owner scale recorded. If it exceeds ten
...
- **Person filter works for photos**: `adapters/db/global-catalog.ts:2132`
...
## F4 — Owner-library run order

**Goal.** Move the owner's real catalog onto the new pipeline, once, in the
only order that is safe.

1. Install the F1–F3 build. First open runs the V16 / v6 migration; confirm
   the backup file exists and the photo observation count drops to zero.
2. `avc faces index <photo root>` (or Analiza → Zdjęcia). Watch
   `photosIndexed` climb; expect roughly the 25k-observation order of
   magnitude the PHOTO LIBRA corpus implies, not equality.
3. `scripts/faces-benchmark.ts` in real-data mode over the freshly-indexed
   catalog; owner labels the emitted pair sample; the report picks the cut.
...
5. `faces recluster` for real. Every person id is re-minted; every name is
   gone.
6. Owner renames the people that matter from the person cards.
...
- **Fable UI review** of Osoby: chips, person card density, the recluster
...
1. **The calibration corpus is machine-made.** PHOTO LIBRA's 2,060 clusters
   are its own greedy output at 0.45/0.50/min-3 with four human actions
   logged. Maximising agreement with it selects for the algorithm being
   replaced. *Mitigation:* the owner-labelled pair sample is the deciding
   criterion (ADR-0017 D4); the libra partition is reported as agreement, not
   as accuracy.
2. **Native detection will not reproduce libra's 25,327 observations.**
...
3. **Rebuild cost at owner scale.** ~50k observations means the sparse edge
   pass dominates. The ten-minute budget and the banding fallback are declared
   in F2; if both miss, the phase stops and the owner decides, rather than the
   threshold being relaxed to make it fast.
4. **Every person id changes and every name is dropped.** Only one name exists
   today, so the cost is small now — but the same rebuild after the owner has
   named fifty people is expensive. A durable name anchor (a name pinned to an
   observation, not to a person id) is the obvious follow-up and is
...
7. **The migration is not revertible by the app.** Only by hand, from the
   NDJSON dump. Accepted: the rows are the ones the owner decided are
   worthless.
...
- **Changelog.** Each of F1–F3 lands its own `[Unreleased]` lines, quoted
  ...
  operational run over the owner's data and adds no line. F5's release commit
  moves the accumulated entries under `## [0.6.25]` and adds the commit links.
  This F0 documentation PR adds no `CHANGELOG.md` line: the repo rule scopes
  changelog entries to behaviour-visible changes, and documentation-only
  commits have no precedent in the file.
