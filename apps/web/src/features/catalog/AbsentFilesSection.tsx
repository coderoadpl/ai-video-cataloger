...
export const AbsentFilesSection = ({ folder }: AbsentFilesSectionProps) => {
  ...
  return (
    <Box sx={{ borderTop: 1, borderColor: 'divider' }} data-testid="absent-files-section">
      <Box
        component="button"
        type="button"
        onClick={() => setOpen((value) => !value)}
        aria-expanded={open}
        data-testid="absent-files-toggle"
        sx={{
          display: 'flex',
          alignItems: 'center',
          gap: 0.75,
      ...
      >
        ...
        <Typography variant="caption" sx={{ fontWeight: 600 }}>
          {dictionary.catalog.absentSectionTitle} ({entries.length})
        </Typography>
      </Box>
