...
export const createMaskedPrompter = (streams: MaskedPromptStreams): MaskedPrompter => {
  ...
  // readline infers `terminal` from the output stream, and this wrapper is not a TTY:
  // without carrying the real stream's verdict it stops taking the input out of cooked
  // mode, and the terminal driver - not readline - echoes the typed secret.
  const terminal = streams.terminal ?? streams.output.isTTY === true;
